...
# MODEL FITTING
# ==============================================================================
def fit_garch_model(daily_returns: pd.Series):
    """
    ...
    # 3. Use structural mean from GARCH fit
    mu = params['mu'] / 100.0
...
def simulate_paths(
    ...
    """
    Simulates price paths using GARCH(1,1) + Student-t + Jumps.
    ...
    for dt in dt_schedule:
        ...
        mu_clamped = np.clip(mu, -DRIFT_CLAMP_MULT * sigma_day_step, DRIFT_CLAMP_MULT * sigma_day_step)

        # 4. Apply Drift (clamped, with jump correction)
