...
|   |-- robustness_sizing.py      #   Mid-edge Kelly -> Baker-McHale -> presence floor -> caps sizing
...
- `robustness_sizing.py` -- sizing overhaul 2026-07-12 (plan-reviewed, temp/mm_sizing_fix_plan.md): Kelly edge prices against the MARKET mid (`ContractSizingInput.mkt_mid`, both-sides-uncrossed guard in the harness; falls back to our own quote when the book is one-sided -- the pre-overhaul behavior). This decouples quote sizes from our own spread calibration (the old self-referential edge collapsed sizes ~10x when the k_arrival recal cut delta_x). Pipeline: fraction space (Kelly -> per-strike Baker-McHale -> bankroll-util cap -> fractional-c, the last FRACTION-SPACE ceiling) then share space (presence floor -> inventory headroom cap -> depth cap -> bucket worst-case cap, a final share-space override -- caps dominate floors). The old joint-ladder stand-in (sum f <= max single f) and fraction-space ruin scale are replaced by the bucket-decomposition worst-case cap (strikes partition outcomes into n+1 intervals; max single-bucket loss <= per_expiry_cap_frac; records RUIN -- `SizingCap.LADDER_JOINT` kept for journal compat, never emitted). Presence floor: `MMConfig.presence_frac` (0.005 of ladder bankroll notional per side, 0 disables) keeps the maker present at zero directional edge, tapered toward each side by inventory (`q`/`q_max`); `SizingCap.INVENTORY` records binding headroom (bid <= q_max - q, ask <= q_max + q); `SizingDecision.max_add_yes/no` carry the headrooms (0.0 = not computed). The DEPTH cap is live: the harness passes `last_liquidity` into `size_ladder`, so quote sizes are bounded by realized depth per side (`SizingCap.DEPTH` in the audit when binding).
