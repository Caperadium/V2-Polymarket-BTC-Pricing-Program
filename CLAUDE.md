...
- `fair_value_anchor.py` -- Beuoy bankroll-credibility consensus probability + Bayes bankroll update. The per-strike sanity bound checks the consensus against the SANITIZED (bucket-round-tripped) pricer/mid ladders, not the raw inputs (fix 2026-07-14: the raw-band check made one crossed mid pair -- routine on thin far-expiry wings -- a guaranteed violation at EVERY strike via the global bucket renormalization, so the anchor was never BEUOY, the harness's 20-clean-tick auto-unfreeze could never start, and that ladder's bankroll froze permanently). Non-monotone input ladders are repaired by the bucket transform (logged at debug); the band check is now a pure numeric safety net.
