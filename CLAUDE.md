...
- `shadow_runner.py` -- Stage-A read-only REST-polling runner (fill-free by construction) plus `resolve_event`/`resolve_next_event` (shared with `paper_runner.py`) and `CachedEngine` (re-price cache over the real pricing engine, GARCH cache refit every `garch_refit_s`, default 6h; bipower-calibrated `jump_params` via `load_calibrated_jumps`, refreshed on the same cadence, falling back to engine defaults if calibration is unavailable/unconverged -- fix 2026-07-10, previously priced on hardcoded default jumps ~+1.2-1.5c rich at ATM).
