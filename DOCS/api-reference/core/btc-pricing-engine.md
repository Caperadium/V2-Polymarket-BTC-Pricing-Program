...
Fit GARCH(1,1) or FIGARCH(1,d,1) + Student-t via `arch` library. When `use_figarch=True`, fits `arch_model(vol='FIGARCH', p=1, q=1)` jointly estimating phi, d, beta. Falls back to GARCH on convergence failure. Returns dict of fitted parameters.
...
| `martingale_anchor` | `False` | -- | Use exponential-cumulant jump compensator (true risk-neutral martingale). Default False keeps the legacy log-mean compensator; flip only with calibration re-baseline. |
...
| Horizon | Model |
|---------|-------|
| T > 90 days | Naive prior only (mu=0, GARCH+t, no jumps) |
